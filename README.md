...
For day-to-day CI, the workflow does **not** override `CreateGitHubDeployRole`, so the stack keeps the parameter value from the last manual or pipeline deploy (typically the role remains managed by the stack).
